...
export const createOrder = async (req: Request, res: Response) => {
  try {
    ...
    io.to(sellerId).emit('order_created', newOrder);
...
export const associateBuyer = async (req: Request, res: Response) => {
  try {
    ...
    if (order.stage !== OrderStage.PLACED) {
      return res.status(400).json({ message: 'Can only associate buyer to Placed orders' });
    }
    ...
    // MANUAL HISTORY PUSH
    order.stageHistory.push({ stage: OrderStage.BUYER_ASSOCIATED, timestamp: new Date() });
